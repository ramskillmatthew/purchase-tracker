...
export async function GET(request: Request) {
  try {
    ...
    const batches = await supabaseRequestAll<ImportBatch>(`ebay_import_batches?owner_id=eq.${ownerId}&status=in.(waiting,processing,completed)&select=id,status,total_count,created_at,updated_at&order=created_at.asc`);
