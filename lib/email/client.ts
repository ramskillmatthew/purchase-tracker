...
export async function countMail(ownerId:string,criteria:EmailSearch,expectedType?:EmailType,providers:Array<"yahoo"|"gmail">=["yahoo","gmail"]){const jobs=[] as Array<Promise<{count:number;foldersSearched:number}>>;if(providers.includes("yahoo")&&process.env.YAHOO_EMAIL)jobs.push(countYahoo(criteria,expectedType));if(providers.includes("gmail")&&(await gmailAccounts(ownerId)).length)jobs.push(countGmail(ownerId,criteria,expectedType));const settled=await Promise.allSettled(jobs);return settled.reduce((total,item)=>item.status==="fulfilled"?{count:total.count+item.value.count,foldersSearched:total.foldersSearched+item.value.foldersSearched}:total,{count:0,foldersSearched:0});}
