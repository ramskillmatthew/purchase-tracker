...
export const LISTING_GENERATION_SYSTEM_PROMPT =
  ...
  + "BRAND: identify the manufacturer only from clear visible evidence in the photos (a logo, label, or embossed marking) — for example Nike, Adidas, On Running, ASICS, New Balance, Hoka, Birkenstock. Never invent a brand you cannot see evidence for — if genuinely unclear, leave the value null. The Swiss running brand with the curved-line logo is always reported as \"On Running\" — its exact, selectable Vinted manufacturer brand — never \"On\" alone, which is not a real Vinted brand option. "
