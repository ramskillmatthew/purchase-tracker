...
-- eBay UK URL importer (Stage One). Imported products remain ordinary
-- listing_drafts; these columns only preserve their provenance.
alter table public.listing_drafts add column if not exists source_type text not null default 'photos'
  check (source_type in ('photos', 'ebay_uk'));
