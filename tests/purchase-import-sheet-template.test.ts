import { describe, expect, it } from "vitest";
import ExcelJS from "exceljs";
import { conditions } from "@/lib/validation/purchase";
import { buildImportTemplate, IMPORT_TEMPLATE_FILENAME } from "@/lib/purchase-import-sheet/template";
import { importColumns, buildImportRows } from "@/lib/purchase-import-sheet/schema";
import { readSheetFile } from "@/lib/purchase-import-sheet/read-file";

describe("buildImportTemplate", () => {
  it("suggests the exact requested filename", () => {
    expect(IMPORT_TEMPLATE_FILENAME).toBe("purchase-import-template.xlsx");
  });

  it("opens successfully and contains the Purchases and Instructions worksheets", async () => {
    const buffer = await buildImportTemplate();
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(buffer as any);
    expect(workbook.getWorksheet("Purchases")).toBeTruthy();
    expect(workbook.getWorksheet("Instructions")).toBeTruthy();
  });

  it("has all eight columns, in the exact required order and wording", async () => {
    const buffer = await buildImportTemplate();
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(buffer as any);
    const sheet = workbook.getWorksheet("Purchases")!;
    const headings = sheet.getRow(1).values as unknown[];
    // ExcelJS row.values is 1-indexed with a leading empty slot
    expect(headings.slice(1)).toEqual(importColumns.map(c => c.heading));
  });

  it("does not pre-fill any example purchase row — the data-entry table itself is empty (rows beyond the header carry validation only, no values)", async () => {
    const buffer = await buildImportTemplate();
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(buffer as any);
    const sheet = workbook.getWorksheet("Purchases")!;
    const row2Values = (sheet.getRow(2).values as unknown[]).slice(1).filter(value => value !== null && value !== undefined);
    expect(row2Values).toEqual([]);
  });

  function columnLetter(field: string): string {
    return String.fromCharCode(65 + importColumns.findIndex(c => c.field === field));
  }

  it("formats Order Date as dd/mm/yyyy, SKU as text, and Price Purchased as GBP currency", async () => {
    const buffer = await buildImportTemplate();
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(buffer as any);
    const sheet = workbook.getWorksheet("Purchases")!;
    expect(sheet.getColumn(columnLetter("order_date")).numFmt).toBe("dd/mm/yyyy");
    expect(sheet.getColumn(columnLetter("sku")).numFmt).toBe("@");
    expect(sheet.getColumn(columnLetter("price_purchased")).numFmt).toContain("£");
  });

  it("freezes the header row and adds an autofilter across all eight columns", async () => {
    const buffer = await buildImportTemplate();
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(buffer as any);
    const sheet = workbook.getWorksheet("Purchases")!;
    expect(sheet.views[0]).toMatchObject({ state: "frozen", ySplit: 1 });
    expect(JSON.stringify(sheet.autoFilter)).toContain("A1");
    expect(JSON.stringify(sheet.autoFilter)).toContain("H1");
  });

  it("has sensible column widths so no heading is clipped", async () => {
    const buffer = await buildImportTemplate();
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(buffer as any);
    const sheet = workbook.getWorksheet("Purchases")!;
    importColumns.forEach(column => {
      const width = sheet.getColumn(columnLetter(column.field)).width ?? 0;
      expect(width).toBeGreaterThanOrEqual(column.heading.length);
    });
  });

  it("adds Yes/No dropdown validation on the Arrived column and the canonical list on Item Condition, sourced from lib/validation/purchase.ts", async () => {
    const buffer = await buildImportTemplate();
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(buffer as any);
    const sheet = workbook.getWorksheet("Purchases")!;
    const arrivedValidation = sheet.getCell("D5").dataValidation;
    expect(arrivedValidation?.type).toBe("list");
    expect(arrivedValidation?.formulae?.[0]).toBe('"Yes,No"');
    const conditionValidation = sheet.getCell("G5").dataValidation;
    expect(conditionValidation?.type).toBe("list");
    expect(conditionValidation?.formulae?.[0]).toBe(`"${conditions.join(",")}"`);
  });

  it("contains no formulas anywhere in the generated workbook", async () => {
    const buffer = await buildImportTemplate();
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(buffer as any);
    for (const sheet of workbook.worksheets) {
      sheet.eachRow(row => row.eachCell(cell => {
        expect(cell.type).not.toBe(ExcelJS.ValueType.Formula);
      }));
    }
  });

  it("includes a short Instructions sheet mentioning the canonical condition list and the no-formulas rule", async () => {
    const buffer = await buildImportTemplate();
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(buffer as any);
    const instructions = workbook.getWorksheet("Instructions")!;
    const text = instructions.getSheetValues().flat().filter(Boolean).join(" ");
    expect(text).toContain("Order Date is required");
    expect(text).toContain("formulas");
    for (const condition of conditions) expect(text).toContain(condition);
  });

  it("REQUIREMENT: the same template can be read back by the importer's own parser after adding valid data", async () => {
    const buffer = await buildImportTemplate();
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(buffer as any);
    const sheet = workbook.getWorksheet("Purchases")!;
    sheet.addRow(["2026-07-24", "Vinted", "1801", "Yes", "Nike Air Max 95", "9", "Brand new", 13.49]);
    const roundTrip = Buffer.from(await workbook.xlsx.writeBuffer());
    const aoa = await readSheetFile(roundTrip, "xlsx");
    const result = buildImportRows(aoa);
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.rows).toHaveLength(1);
      expect(result.rows[0].errors).toEqual([]);
      expect(result.rows[0].purchase?.sku).toBe("1801");
    }
  });
});
