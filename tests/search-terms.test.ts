...
describe("forgiving mailbox terms",()=>{
  ...
  it("expands order-confirmation intent without requiring an exact subject",()=>{const values=searchVariants(["order confirmation"]);expect(values).toContain("your order");expect(values).toContain("order received");expect(values).toContain("receipt");});
  it("canonicalizes a misspelled accented sender name",()=>{expect(canonicalSender("Pokemon Centre")).toBe("pokémon center");});
  it("recognizes order lifecycle intent independently of retailer",()=>{expect(semanticSubjectTerms(["order confirmation"])).toContain("order");expect(semanticSubjectTerms(["find my receipt"])).toContain("invoice");expect(semanticSubjectTerms(["tracking update"])).toContain("tracking");expect(semanticSubjectTerms(["delivery notice"])).toContain("delivered");expect(semanticSubjectTerms(["cancelled order"])).toContain("cancellation");expect(semanticSubjectTerms(["refund update"])).toContain("refunded");});
  it("treats standalone purchase and order words as semantic intent",()=>{expect(semanticSubjectTerms(["purchase"])).toContain("order");expect(semanticSubjectTerms(["latest order"])).toContain("receipt");});
  it("searches receipt requests using retailer confirmation wording",()=>{const terms=semanticSubjectTerms(["latest receipt"]);expect(terms).toContain("confirmed");expect(terms).toContain("confirmation");});
  it("counts the broad vocabulary retailers use for transactions",()=>{const terms=semanticSubjectTerms(["order confirmations"]);for(const term of ["purchase","receipt","order details","payment received","order"])expect(terms).toContain(term);});
  it("does not count preorder promotions as order confirmations",()=>{const terms=semanticSubjectTerms(["order confirmations"]);expect(terms).not.toContain("preorder");expect(terms.slice(0,3)).toEqual(["confirmed","confirmation","thank you for"]);});
  it("does not treat generic order wording as proof of confirmation",()=>{const terms=countSubjectTerms(["order confirmations"]);expect(terms).not.toContain("order");expect(terms).toContain("order details");expect(terms).toContain("purchase");expect(terms).toContain("receipt");});
  it("prioritizes confirmation wording over generic order wording",()=>{expect(semanticSubjectTerms(["latest order confirmation"]).slice(0,3)).toEqual(["confirmed","confirmation","thank you for"]);});
