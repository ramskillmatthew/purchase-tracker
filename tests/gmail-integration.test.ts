...
describe("Gmail integration",()=>{
  ...
  it("treats Gmail and Yahoo as provider selectors rather than sender terms",()=>{const source=read("lib/anthropic/assistant.ts");expect(source).toContain('providers: MailProvider[] = /\\bgmail\\b/i.test(message)');expect(source).toContain('providerNeutralMessage');expect(source).toContain('providers.length === 1 && providers[0] === "yahoo"');expect(source).not.toContain('description: "Search Yahoo Mail');});
});
